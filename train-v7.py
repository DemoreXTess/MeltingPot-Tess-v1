...
if __name__ == "__main__":
    ...
    for update in range(num_updates):
        ...
        with torch.no_grad():
            ...
            val_plus1 = agent.get_value(batch_o, history=(h,c), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, var=config.var, **pri_kwargs).view(num_envs,act_s[0])
            
            cho_done = done
            returns = torch.zeros((num_steps,num_envs,act_s[0]), device="cuda")
            for t in reversed(range(num_steps)):
                if t == num_steps - 1:
                    nextnonterminal = (1.0 - cho_done)
                    nextreturn = val_plus1 * torch.tensor([vec_dev[i] for i in tasks for _ in range(act_s[0])],device="cuda").view(num_envs,act_s[0]) +\
                          torch.tensor([vec_means[i] for i in tasks for _ in range(act_s[0])],device="cuda").view(num_envs,act_s[0])
                else:
                    nextnonterminal = 1.0 - roll_dones[t+1]
                    nextreturn = returns[t + 1]
                returns[t] = roll_rew[t] + gamma * nextreturn * nextnonterminal 
            advantages = returns - (roll_val * torch.tensor([vec_dev[i] for i in tasks for _ in range(act_s[0])],device="cuda").view(num_envs,act_s[0]) +\
                          torch.tensor([vec_means[i] for i in tasks for _ in range(act_s[0])],device="cuda").view(num_envs,act_s[0]))
        ...
        for ith_e in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                mb_time = b_time[minibatch_ind].view(-1,num_envs,act_s[0])
                h__0 = [torch.zeros((num_envs,act_s[0],agent.last_layer),dtype=torch.float32,device="cuda").view(-1,agent.last_layer)]
                c__0 = [torch.zeros((num_envs,act_s[0],agent.last_layer),dtype=torch.float32,device="cuda").view(-1,agent.last_layer)]
        ...
        mean_returns = torch.mean(returns.view(num_steps,num_envs,act_s[0]),dim=0).view(-1) # Shape num_envs
        #dev_returns = torch.std(returns,dim=0) # Shape num_envs

        for ind,i in enumerate(tasks):
            for u in range(act_s[0]):
                temp_means[i] = (1-beta) * temp_means[i] + beta * mean_returns[ind*act_s[0]+u]
                temp_vs[i] = (1-beta) * temp_vs[i] + beta * torch.square(mean_returns[ind*act_s[0]+u])
