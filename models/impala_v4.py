...
class Model(nn.Module):
    ...
    def __init__(self, obs_space, act_space, timestep_layer=64, last_layer=256, device="cuda",inv=False, **kwargs):
        ...
        self.harmony = nn.Linear(self.last_layer+timestep_layer,256)
        if inv:
            output_size=output_size+16
    ...
    def get_action(self, x, shoot, history, timestep, inv=False, reduce=True, **kwargs):
    ...
    def sample_act_and_value(self, x, history, timestep, **kwargs):

        value_aug = self.get_value_with_augmentation(x, history, timestep=timestep, **kwargs)
    ...
    def check_action_and_value(self, x, act, history, timestep, inv=False):
    ...
    def get_value(self, x, timestep, history):
