...
if __name__ == "__main__":
    ...
    batch_size = num_envs * num_steps * (16 - config.bot)
    ...
    done = torch.tensor([0]*num_envs,device="cuda").view(-1,1).expand(-1,agent_count)
    ...
    roll_o = torch.zeros((num_steps, num_envs, agent_count,)+shape, device="cuda")
    roll_a = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_lp = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_rew = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_dones = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_val = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_time = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    roll_shoot = torch.zeros((num_steps, num_envs, agent_count), device="cuda")
    ...
    #LSTM helper
    h_0 = torch.zeros((num_envs, agent_count, agent.last_layer),device="cuda",dtype=torch.float32)
    c_0 = torch.zeros((num_envs, agent_count, agent.last_layer),device="cuda",dtype=torch.float32)
    #Bot
    h_bot = torch.zeros((num_envs, bot_count, agent.last_layer),device="cuda",dtype=torch.float32)
    c_bot = torch.zeros((num_envs, bot_count, agent.last_layer),device="cuda",dtype=torch.float32)
    ...
    last_act_scores = np.zeros((num_envs,agent_count))
    ...
    for update in range(num_updates):
        ...
        for step in range(num_steps):
            ...
            roll_o[step] = obs[:,bot_count:,:,:,:]
            roll_shoot[step] = ready_shoot_info[:,bot_count:]
            ...
            with torch.no_grad():
                ...
                batch_shoot = ready_shoot_info[:,bot_count:].reshape(-1)
            ...
            value = value.view(num_envs,agent_count,task_count)
            ...
            bot_obs = obs[:,:bot_count,:,:,:].reshape((-1,)+shape)
            with torch.no_grad():
                bot_act, (h_bot, c_bot), _ = bots.get_action(bot_obs, shoot=ready_shoot_info[:,:bot_count].reshape(-1), history=(h_bot.view(-1,agent.last_layer),c_bot.view(-1,agent.last_layer)))

            bot_act = bot_act.view(num_envs,bot_count)
            
            new_acts = torch.concatenate((bot_act,act),dim=1)
            ...
            for ind,i in enumerate(done):
                if i:
                    ...
                    last_act_scores[ind] = real_[bot_count:]
                    last_scores[ind] = real_[bot_count:].sum()
            ...
            done = torch.from_numpy(done).to("cuda").view(-1,1).expand(-1,agent_count)
        ...
        with torch.no_grad():
            ...
            val_plus1 = agent.get_value(batch_o, history=(h,c), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, var=config.var, **pri_kwargs).view(num_envs,agent_count)
            
            cho_done = done
            returns = torch.zeros((num_steps,num_envs,agent_count), device="cuda")
            for t in reversed(range(num_steps)):
                if t == num_steps - 1:
                    nextnonterminal = (1.0 - cho_done)
                    nextreturn = val_plus1 * torch.tensor([vec_dev[i] for i in tasks for _ in range(agent_count)],device="cuda").view(num_envs,agent_count) +\
                          torch.tensor([vec_means[i] for i in tasks for _ in range(agent_count)],device="cuda").view(num_envs,agent_count)
                else:
                    nextnonterminal = 1.0 - roll_dones[t+1]
                    nextreturn = returns[t + 1]
                returns[t] = roll_rew[t] + gamma * nextreturn * nextnonterminal 
            advantages = returns - (roll_val * torch.tensor([vec_dev[i] for i in tasks for _ in range(agent_count)],device="cuda").view(num_envs,agent_count) +\
                          torch.tensor([vec_means[i] for i in tasks for _ in range(agent_count)],device="cuda").view(num_envs,agent_count))
        ...
        for ith_e in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                mb_obs = b_obs[minibatch_ind].view((-1,num_envs,agent_count,)+shape)
                mb_time = b_time[minibatch_ind].view(-1,num_envs,agent_count)
                h__0 = [torch.zeros((num_envs,agent_count,agent.last_layer),dtype=torch.float32,device="cuda").view(-1,agent.last_layer)]
                c__0 = [torch.zeros((num_envs,agent_count,agent.last_layer),dtype=torch.float32,device="cuda").view(-1,agent.last_layer)]
                ...
                with torch.no_grad():
                    for i in range(mb_obs.shape[0] - 1):
                        ...
                        for index,done_info in enumerate(roll_dones[start//num_envs//act_s[0]+i+1]):
                            if done_info[0] == 1:
                                h_n[index] *= 0
                                c_n[index] *= 0
                        h__0.append(h_n)
        ...
        mean_returns = torch.mean(returns.view(num_steps,num_envs,agent_count),dim=0).view(-1) # Shape num_envs
        #dev_returns = torch.std(returns,dim=0) # Shape num_envs

        for ind,i in enumerate(tasks):
            for u in range(agent_count):
                temp_means[i] = (1-beta) * temp_means[i] + beta * mean_returns[ind*agent_count+u]
                temp_vs[i] = (1-beta) * temp_vs[i] + beta * torch.square(mean_returns[ind*agent_count+u])
