...
class TessEnv(gym.Env):
    ...
    def reset(self):
        ...
        if self.name == "clean_up":
            ...
            self.consume_histories = [queue.Queue(maxsize=10) for _ in range(self.num_players)]
            for ind,i in enumerate(self.clean_up_histories):
                for _ in range(10):
                    self.consume_histories[ind].put(0)
                    i.put(0)
            self.clean_rewards = [0] * self.num_players
            self.consume_rewards = [0] * self.num_players
        ...
        if "territory" in self.name:
            ...
            self.wait = np.zeros((self.num_players,))
        ...
        if "clean_up" in self.name:
            ...
            if event[0] == "collected_resource":
                ...
                if resource == 1:
                    ...
                    self.rewards[player_id-1] -= .25

            if event[0] == "interaction":
                
                player_id = int(event[1][2])-1
                player_id2 = int(event[1][4])-1
                reward = float(event[1][6])
                reward2 = float(event[1][8])
                inv = event[1][10]
                inv2 = event[1][12]
                if inv[0] > inv[1]:
                    self.rewards[player_id] += .5
                if inv2[0] > inv2[1]:
                    self.rewards[player_id2] += .25
