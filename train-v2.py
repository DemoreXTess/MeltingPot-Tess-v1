...
from augmentation import multiply_by_rand, cutout_color
...
def parse_args():
    ...
    parser.add_argument("--model",type=str,default="impala-v2")
    parser.add_argument("--train-config",type=str,default="ImpalaConfig")
    parser.add_argument("--env-version",type=str,default="TessEnv-v2")
...
if __name__ == "__main__":
    ...
    #Data Augmentation
    agent.set_augmentation_func(cutout_color)
    ...
    if config.optimizer == "rmsprop":
        optimizer = RMSprop(agent.parameters(), lr=lr)
    ...
    for update in range(num_updates):
        ...
        for step in range(num_steps):
            ...
            with torch.no_grad():
                ...
                act, log_prob, value, (h_0, c_0), value_aug = agent.sample_act_and_value(batch_o, history=(h_0,c_0), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, **pri_kwargs)
        ...
        with torch.no_grad():
            ...
            h_0 = h_0.view(-1,agent.last_layer)
            c_0 = c_0.view(-1,agent.last_layer)
            val_plus1 = agent.get_value_with_augmentation(batch_o, history=(h_0,c_0), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, **pri_kwargs).view(num_envs,-1)
        ...
        for ith_e in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                loss =  pg_loss + v_loss * config.v_coef - ent_loss * ent_coef
