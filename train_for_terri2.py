...
def parse_args():
    ...
    parser.add_argument("--model",type=str,default="impala_v5")
...
if __name__ == "__main__":
    ...
    roll_time = torch.zeros((num_steps, num_envs), device="cuda")
    roll_shoot = torch.zeros((num_steps, num_envs), device="cuda")
    roll_infos = torch.zeros((num_steps, num_envs, agent.last_layer), device="cuda")
    ...
    h_hier = torch.zeros((num_envs, act_s[0], agent.last_layer),device="cuda",dtype=torch.float32)
    c_hier = torch.zeros((num_envs, act_s[0], agent.last_layer),device="cuda",dtype=torch.float32)
    ...
    for update in range(num_updates):
        ...
        for step in range(num_steps):
            ...
            time_d = torch.tensor(env.get_attr("time"),device="cuda")
            shoot = torch.tensor(env.get_attr("shoot"),device="cuda")
            ...
            roll_shoot[step] = torch.stack([shoot[ind,i] for ind,i in enumerate(random_players)],dim=0)
            ...
            with torch.no_grad():
                ...
                time_d = time_d.reshape(-1,1).expand(-1,act_s[0]).reshape(-1,1)
                batch_o = obs.view((-1,)+shape)
                h_0 = h_0.view((-1,)+(agent.last_layer,))
                c_0 = c_0.view((-1,)+(agent.last_layer,))
                h_hier = h_hier.view((-1,)+(agent.last_layer,))
                c_hier = c_hier.view((-1,)+(agent.last_layer,))
                shoot = shoot.reshape(-1,1)
                act, log_prob, value, (h_0, c_0), info, (h_hier, c_hier) = agent.sample_act_and_value(batch_o, history=(h_0,c_0), timestep=time_d, shoot=shoot,\
                    hiers=(h_hier, c_hier),\
            ...
            info = info.view(num_envs,act_s[0],agent.last_layer)
            roll_infos[step] = torch.stack([info[index,i] for index,i in enumerate(random_players)])

            h_0 = h_0.view(num_envs,act_s[0],agent.last_layer)
            c_0 = c_0.view(num_envs,act_s[0],agent.last_layer)
            h_hier = h_hier.view(num_envs,act_s[0],agent.last_layer)
            c_hier = c_hier.view(num_envs,act_s[0],agent.last_layer)

            act2 = torch.zeros(size=(2,act_s[0]))
            act2[0,random_players[2]] = act[2,random_players[2]]
            act2[1,random_players[3]] = act[3,random_players[3]]
            act[2:,:] = act2
            ...
            for ind,i in enumerate(done):
                if i:
                    ...
                    if float(real_.sum()) > save_stat_score:
                        save_stat_score=float(real_.sum())
                        save=True
                    last_scores[ind] = np.sum(real_)
        ...
        with torch.no_grad():
            ...
            time_d = torch.tensor(env.get_attr("time"),device="cuda").view(num_envs,1)
            obs_tuple = [obs[ind,i,:,:,:] for ind,i in enumerate(random_players)]

            h_hie = torch.stack([h_hier[index,i,:] for index,i in enumerate(random_players)]).view(-1,agent.last_layer)
            c_hie = torch.stack([c_hier[index,i,:] for index,i in enumerate(random_players)]).view(-1,agent.last_layer)

            val_plus1 = agent.get_value(torch.stack(obs_tuple,dim=0), timestep=time_d, history=(h_hie,c_hie), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, var=config.var, **pri_kwargs).view(num_envs,-1)
        ...
        b_infos = roll_infos.reshape(-1,agent.last_layer)
        if "prisoner" in args.substrate:
        ...
        for ith_e in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                if start == 0:
                    hier_h = [torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")]
                    hier_c = [torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")]
                    ...
                    with torch.no_grad():
                        if pri_kwargs["inv"]:
                            pre_inv = b_invs[start-config.burn_in*num_envs:start].view(-1,num_envs,act_s[0],2)
                        else:
                            pre_inv = False
                        pre_obs = b_obs[start-config.burn_in*num_envs:start].view((-1,num_envs,)+shape)
                        pre_h = torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")
                        pre_c = torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")
                        pre_hier_h = torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")
                        pre_hier_c = torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")
                        pre_dones = roll_dones[start//num_envs-config.burn_in:start//num_envs+1]
                        pre_switch = roll_switch[start//num_envs-config.burn_in:start//num_envs+1]
                        pre_time = b_time[start-config.burn_in*num_envs:start].view(-1,num_envs)
                        for i in range(pre_obs.shape[0]):
                            if config.burn_in == 0:
                                print("BUG")
                            if type(pre_inv) != bool:
                                _ , (pre_h,pre_c), _, (pre_hier_h,pre_hier_c) = agent.forward(pre_obs[i].view((-1,)+shape),timestep=pre_time[i].view(-1,1),history=(pre_h,pre_c),hier_history=(pre_hier_h,pre_hier_c),inv=pre_inv[i].view(-1,2))
                            else:
                                _ , (pre_h,pre_c), _, (pre_hier_h,pre_hier_c) = agent.forward(pre_obs[i].view((-1,)+shape),timestep=pre_time[i].view(-1,1),history=(pre_h,pre_c),hier_history=(pre_hier_h,pre_hier_c))
                            for index,done_info in enumerate(pre_dones[i+1]):
                                if done_info.item() == 1:
                                    ...
                                    pre_hier_h[index] *= 0
                                    pre_hier_c[index] *= 0
                            for index,switch_info in enumerate(pre_switch[i+1]):
                ...
                with torch.no_grad():
                    for i in range(mb_obs.shape[0] - 1):
                        ...
                        if pri_kwargs["inv"]:
                            _, (h_n,c_n), _, (hier_h_n, hier_c_n) = agent.forward(mb_obs[i].view((-1,)+shape),timestep=mb_time[i].view(-1,1),history=(h__0[-1],c__0[-1]),hier_history=(hier_h[-1],hier_c[-1]),inv=mb_invs[i].view(-1,2))
                        else: 
                            _, (h_n,c_n), _, (hier_h_n, hier_c_n) = agent.forward(mb_obs[i].view((-1,)+shape),timestep=mb_time[i].view(-1,1),history=(h__0[-1],c__0[-1]),hier_history=(hier_h[-1],hier_c[-1]))
                        ...
                        h__0.append(h_n)
                        c__0.append(c_n)
                        hier_h.append(hier_h_n)
                        hier_c.append(hier_c_n)

                h__0 = torch.concatenate(h__0,dim=0)
                c__0 = torch.concatenate(c__0,dim=0)
                hier_h = torch.concatenate(hier_h,dim=0)
                hier_c = torch.concatenate(hier_c,dim=0)
                ...
                mb_shoot = b_shoot[minibatch_ind]
                mb_infos = b_infos[minibatch_ind]

                new_logprob, entropy, value = agent.check_action_and_value(mb_obs,mb_infos,\
                                            mb_actions, history=(h__0,c__0), hiers=(hier_h,hier_c), timestep=mb_time, shoot=mb_shoot, **pri_kwargs)          
