...
if __name__ == "__main__":
    ...
    env = SubprocVecEnv([lambda: gym.make(args.env_version,\
                        ...
                        name=args.substrate, use_event=i, env_count=3)\
                        for i in ids])
    ...
    act_s = env.action_space["action"].shape
    ...
    agg_m = model(obs_s[1:], num_act, 1 ,inv="prisoner" in args.substrate)
    agg_m = agg_m.to("cuda")
    ...
    agg_h = torch.zeros((5, act_s[0], agent.last_layer),device="cuda",dtype=torch.float32)
    agg_c = torch.zeros((5, act_s[0], agent.last_layer),device="cuda",dtype=torch.float32)
    ...
    for update in range(num_updates):
        ...
        for step in range(num_steps):
            ...
            agg_bo = batch_o[act_s[0]*5:act_s[0]*5*2]
            agg_so = batch_shoot[act_s[0]*5:act_s[0]*5*2]

            agg_act, _, _, (agg_h, agg_c) = agg_m.sample_act_and_value(agg_bo, history=(agg_h.view((-1,)+(agent.last_layer,)),\
                                                                                        agg_c.view((-1,)+(agent.last_layer,))),\
                                                                                        ready_to_shoot=agg_so)
            act2 = agg_act.view(5,act_s[0])

            agg_h = agg_h.view(5,act_s[0],agent.last_layer)
            agg_c = agg_c.view(5,act_s[0],agent.last_layer)

            #Depends on the training situtations
            """act2 = torch.randint(0,num_act+1,size=(5,act_s[0]))
            act2[act2 >= num_act] = 7"""
            for i in range(5,10):
                act2[i-5,last_choosens[i]] = act[i,last_choosens[i]]
            
            #act2[3,last_choosens[3]] = act[3,last_choosens[3]]
            act3 = torch.zeros((6,act_s[0]))
            for i in range(10,16):
                act3[i-10,last_choosens[i]] = act[i,last_choosens[i]]
             
            act[5:10,:] = act2
            act[10:,:] = act3

            real_act = []
            for env_id, action in zip(tasks, act):
                real_act.append({"env_id":env_id,"action":action})
            
            obs, rew, done, info = env.step(real_act)
            ...
            for ind,i in enumerate(done):
                if i:
                    ...
                    last_act_scores[ind] = real_[last_choosens[ind]]
                    last_scores[ind] = real_[last_choosens[ind]]
