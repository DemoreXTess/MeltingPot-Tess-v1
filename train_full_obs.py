...
if __name__ == "__main__":
    ...
    full_obs = np.array(env.get_attr("full_obs"))
    full_obs = torch.from_numpy(full_obs)
    full_shape = full_obs.shape
    full_shape = (full_shape[-1],)+full_shape[-3:-1]
    print(full_shape)

    #Setting Up Model Instance
    
    model = importlib.import_module(f"models.{args.model}").Model
    agent = model(obs_s[1:], num_act, full_shape, inv="prisoner" in args.substrate)
    target_agent = model(obs_s[1:], num_act, full_shape, inv="prisoner" in args.substrate).to("cuda")
    agent = agent.to("cuda")
    ...
    batch_size = num_envs * (num_steps -112)
    ...
    roll_fullo = torch.zeros((num_steps, num_envs,)+full_shape, device="cuda")
    ...
    h_1 = torch.zeros((num_envs, agent.last_layer),device="cuda",dtype=torch.float32)
    c_1 = torch.zeros((num_envs, agent.last_layer),device="cuda",dtype=torch.float32)
    ...
    for update in range(num_updates):
        ...
        for step in range(num_steps):
            ...
            with torch.no_grad():
                ...
                batch_full = full_obs.view((-1,)+full_shape)
                h_0 = h_0.view((-1,)+(agent.last_layer,))
                c_0 = c_0.view((-1,)+(agent.last_layer,))
                h_1 = h_1.view((-1,)+(agent.last_layer,))
                c_1 = c_1.view((-1,)+(agent.last_layer,))
                act, log_prob, value, (h_0, c_0), (h_1, c_1) = agent.sample_act_and_value(batch_o, batch_full, history=(h_0,c_0),
                    history2=(h_1, c_1),\
            ...
            h_1 = h_1.view(num_envs,agent.last_layer)
            c_1 = c_1.view(num_envs,agent.last_layer)

            act2 = torch.zeros(size=(4,act_s[0]))
            ...
            full_obs = torch.from_numpy(np.array(env.get_attr("full_obs")))
            full_obs = full_obs\
                .permute(0,3,1,2)\
                .to("cuda")
        ...
        with torch.no_grad():
            ...
            val_plus1 = agent.get_value(full_obs, history=(h_1,c_1), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, var=config.var, **pri_kwargs).view(num_envs,-1)
        ...
        b_obs = roll_o.reshape((-1,)+shape)[:-num_envs*112]
        b_act = roll_a.reshape(-1)[:-num_envs*112]
        b_logprobs = roll_lp.reshape(-1)[:-num_envs*112]
        b_returns = returns.reshape(-1)[:-num_envs*112]
        b_adv = advantages.reshape(-1)[:-num_envs*112]
        b_val = roll_val.reshape(-1)[:-num_envs*112]
        b_switch = roll_switch.reshape(-1)[:-num_envs*112]
        b_full = roll_fullo.reshape((-1,)+full_shape)[:-num_envs*112]
        if "prisoner" in args.substrate:
        ...
        for ith_e in range(epoch):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                mb_full = b_full[minibatch_ind].view((-1,num_envs,)+full_shape)
                h__1 = [torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")]
                c__1 = [torch.zeros((num_envs,agent.last_layer),dtype=torch.float32,device="cuda")]
                ...
                with torch.no_grad():
                    for i in range(mb_obs.shape[0] - 1):
                        if pri_kwargs["inv"]:
                            (h_n,c_n),(h_n2, c_n2) = agent.act_and_value(mb_obs[i].view((-1,)+shape),mb_full[i].view((-1,)+full_shape),history=(h__0[-1],c__0[-1]),history2=(h__1[-1],c__1[-1]),inv=mb_invs[i].view(-1,2))
                        else: 
                            (h_n,c_n),(h_n2, c_n2) = agent.act_and_value(mb_obs[i].view((-1,)+shape),mb_full[i].view((-1,)+full_shape),history=(h__0[-1],c__0[-1]),history2=(h__1[-1],c__1[-1]))
                ...
                h__1 = torch.concatenate(h__1,dim=0)
                c__1 = torch.concatenate(c__1,dim=0)
                
                mb_obs = mb_obs.view((-1,)+shape)
                mb_full = mb_full.view((-1,)+full_shape)
                ...
                new_logprob, entropy, value = agent.check_action_and_value(mb_obs, mb_full,\
                        mb_actions, history=(h__0,c__0), history2=(h__1,c__1), **pri_kwargs)          
